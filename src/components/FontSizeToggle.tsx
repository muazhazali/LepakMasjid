...
export const FontSizeToggle = () => {
  const { fontSize, increaseFontSize, decreaseFontSize, resetFontSize } = useFontSizeStore();
  const { t } = useTranslation();

  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon" aria-label={t('accessibility.font_size')}>
          <Type className="h-5 w-5" />
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="w-48">
        <DropdownMenuLabel>{t('accessibility.font_size')}: {fontSize}px</DropdownMenuLabel>
        <DropdownMenuSeparator />
