  ...
  async createCustom(data: {
    ...
    try {
      ...
      if (!/^[a-z0-9_]+$/.test(data.key)) {
        throw new Error('Invalid amenity key format. Use lowercase letters, numbers, and underscores only.');
      }
      
      // Check if key already exists
      try {
        const existing = await pb.collection('amenities').getFirstListItem(`key="${data.key}"`);
        if (existing) {
...
export const mosqueAmenitiesApi = {
  ...
  async replaceAll(mosqueId: string, amenities: Array<{
    amenity_id?: string | null;
    details?: MosqueAmenityDetails;
    verified?: boolean;
  }>): Promise<MosqueAmenity[]> {
